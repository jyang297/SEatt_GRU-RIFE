...
class Model:
    ...
    def update(self, allframes, learning_rate=0, mul=1, training=True, flow_gt=None):
        ...
        if training:
            ...
            self.eval()
        flow_list, mask_list, output_allframes, flow_teacher_list, output_onlyteacher, Sum_loss_ssimd, Sum_loss_context, Sum_loss_mse = self.flownet(allframes)

        if training:
            torch.autograd.set_detect_anomaly(True)
            self.optimG.zero_grad()
            loss_G = Sum_loss_context*0.5 + Sum_loss_mse + Sum_loss_ssimd*0.1
            loss_G.backward()
