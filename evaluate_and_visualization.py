...
frame_path = root + "/outputs_01"
output_path = root + "/outputs_0001"
...
def save_model(self, path, rank=0):
    if not os.path.exists(path):
        os.makedirs(path)
    if rank == 0:
        torch.save(self.flownet.state_dict(),'{}/flownet.pth'.format(path))
...
input_tensor = preprocess_frames(frames)
traced_model = torch.jit.trace(model.flownet, input_tensor)
traced_model.save("{}/flownet.pt".format("."))